...
if (haveLogin()) {
    if (isset($_GET['id']) && isset($_GET['q'])) {
        ...
        $search = "PHONE_MODEL LIKE '%$q%' OR STACK_TRACE LIKE '%$q%'";
        
        $res = mysql_query("SELECT count(STACK_TRACE) FROM reports WHERE PACKAGE_NAME ='$package' AND  APP_VERSION_CODE = $vercode AND RESOLVED = 0 AND ($search) GROUP BY STACK_TRACE");
        ...
        $res = mysql_query("SELECT STACK_TRACE,count(STACK_TRACE) FROM reports WHERE PACKAGE_NAME ='$package' AND APP_VERSION_CODE = $vercode AND RESOLVED = 0  AND ($search) GROUP BY STACK_TRACE ORDER BY $order LIMIT $q, 10");
