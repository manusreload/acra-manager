...
if (!haveLogin()) {
    ...
    {
        $rid = mysqli_real_escape_string($core->conn->mysql_conn, $_REQUEST['rid']);
        $res = mysqli_query($core->conn->mysql_conn, "SELECT * FROM reports WHERE id = '$rid'");
        if($row = mysqli_fetch_array($res))
        {
            $url = sprintf("details.php?app=%s&vercode", $row['app']);
    ...
    if (isset($_REQUEST['app']) && isset($_REQUEST['vercode']) && isset($_REQUEST['id'])) {
        ...
        $q = mysqli_real_escape_string($core->conn->mysql_conn, $_REQUEST['id']);
        ...
        $q = mysqli_real_escape_string($core->conn->mysql_conn, $_GET['id']);
        if (isset($_GET['order']) && $_GET['order'] == 'date') {
            $res = $core->conn->query( "SELECT STACK_TRACE  FROM ( SELECT * FROM reports WHERE PACKAGE_NAME ='$package' AND APP_VERSION_CODE = $vercode  ORDER BY USER_CRASH_DATE DESC) t GROUP BY t.STACK_TRACE ORDER BY t.USER_CRASH_DATE DESC LIMIT $q, 1
                       ");
        }
        else {
            $res = $core->conn->query("SELECT STACK_TRACE FROM reports WHERE PACKAGE_NAME ='$package' AND APP_VERSION_CODE = $vercode AND RESOLVED = 0 AND STACK_TRACE_HASH = '$q' GROUP BY STACK_TRACE ORDER BY STACK_TRACE DESC");

//            if (!$res)
//                die(mysqli_error($core->conn->mysql_conn));
        }
        ...
        if ($row = mysqli_fetch_array($res)) {
            ...
            {
                $update = mysqli_query($core->conn->mysql_conn, "UPDATE reports SET `RESOLVED` = '1' WHERE PACKAGE_NAME ='$package' AND APP_VERSION_CODE = $vercode AND STACK_TRACE = '" . $row['STACK_TRACE'] . "'");
                if(!$update) echo mysqli_error ($core->conn->mysql_conn);
            }
            $res2 = mysqli_query($core->conn->mysql_conn, "SELECT REPORT_ID FROM reports WHERE PACKAGE_NAME ='$package' AND APP_VERSION_CODE = $vercode AND STACK_TRACE = '" . $row['STACK_TRACE'] . "' ORDER BY USER_CRASH_DATE DESC");
            $t = 0;
            $first = NULL;
            $info = [
                    'RESOLVED' => false
            ];
            while ($rowPre = mysqli_fetch_array($res2)) {
                $res3 = $core->conn->query("SELECT * FROM reports WHERE REPORT_ID = '{$rowPre['REPORT_ID']}'");
                ...
                <script>
                    function loadapp(id, vercode) {
                        ajaxLoad({
                            ...
                            url: "ajax/GetError.php?id=" + id + "&vercode=" + vercode + "&page=<?php echo CDMCore::filterRequest('page'); ?>"
                        });
                    }
